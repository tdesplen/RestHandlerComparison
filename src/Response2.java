public class Response2 {
        ...
        public ResponseEntity<?> getUsers(
            ...
            try {
                ...
                return ResponseEntity
                        ...
                        .body(Map.of("error", "Invalid request parameters: " + e.getMessage())); // 400

            } catch (AccessDeniedException e) {
                return ResponseEntity
                        .status(HttpStatus.FORBIDDEN)
                        .body(Map.of("error", "Access denied: " + e.getMessage())); // 403
    ...
    public class UserService {
        ...
        public List<UserDTO> getUsers(String name, UUID companyId) {
            ...
            // Four cases: neither, name only, companyId only, or both
            if (name != null && companyId != null) {
                users = userRepository.findByNameContainingAndCompanyId(name, companyId);
            } else if (name != null) {
    ...
    public interface UserRepository extends JpaRepository<User, UUID> {
        ...
        // Matches name as a partial or full substring in firstName or lastName (case-insensitive)
        ...
        @Query("SELECT u FROM User u WHERE " +
                ...
                "LOWER(u.firstName) LIKE LOWER(CONCAT('%', :name, '%')) OR " +
                "LOWER(u.lastName) LIKE LOWER(CONCAT('%', :name, '%')))")
        List<User> findByNameContainingAndCompanyId(@Param("name") String name,
